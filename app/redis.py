...
class RedisPool:
    ...
    async def increase_redis_request_ip(self, ip):
        existing_redis_ip = await self.get_redis_value(ip)
    ...
    async def store_redis_token(self, user, token):
    ...
    async def remove_redis_token(self, user):
        redis_key = user.email
